...
;; 09-Apr-1997		Oscar Figueiredo
;;    EFS compatibility added (EFS replaces ange-ftp in XEmacs 19.15)
...
;;    Backquote use of double-quote inside of a string.  Don't delete
;;    original tar file buffers.  Keep 'em around.
...
(if (and (or (featurep 'ange-ftp)
	     (featurep 'efs))
	 (fboundp 'find-compressed-version))
    (progn
      (message
...
(defun remote-exists-p (file)
  ...
  (if (not (or (featurep 'jka-compr)
	...
	(require 'crypt)))
  (cond
   ((fboundp 'ange-ftp-file-exists-p)
    (ange-ftp-file-exists-p (concat "/anonymous@" file)))
   ((fboundp 'efs-file-exists-p (concat "/anonymous@" file)))))
...
(defun lisp-dir-verify (name)
  "Verifies the archive location of the NAMEd package using ange-ftp or EFS."
  (interactive (list
		(read-string
