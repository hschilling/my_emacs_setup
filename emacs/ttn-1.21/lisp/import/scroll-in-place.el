;; ttn scroll-in-place.el,v 1.2.1.2 1998/01/03 22:13:36 ttn Rel

;;;; -*-Emacs-Lisp-*- Improved Vertical Scrolling Commands
;;;; Written by Eric Eide, last modified on 1998/01/03 22:13:36.
...
;;;; 03-Aug-1994|1.2|~/misc/scroll-in-place.el.Z|
...
;;;;   `scroll-up', and `scroll-other-window') use the "in place" features
;;;;   listed here.  When `scroll-in-place' is `nil', the standard GNU Emacs
...
;;;;   behavior when the minibuffer window is the selected window.  See below.)
;;;;
...
;;;; commands (`scroll-down', `scroll-up', and `scroll-other-window') use the
;;;; "in place" features listed above.  When `scroll-in-place' is `nil', the
;;;; standard GNU Emacs scrolling commands essentially just call the original
;;;; versions of themselves.  (Note that even when `scroll-in-place' is `nil',
;;;; the new versions of `scroll-down' and `scroll-up' have slightly different
;;;; behavior when the minibuffer window is the selected window.  See the
;;;; feature list above.)
;;;;
;;;; NOTE that this package redefines the standard GNU Emacs commands `scroll-
;;;; down', `scroll-up', and `scroll-other-window' (in order to check the
;;;; variable `scroll-in-place', as described above).
;;;;
...
;;;; scrolling commands (`scroll-down', `scroll-up', and `scroll-other-window')
;;;; to use the "in place" features.  If you would rather not have this, set
;;;; the variable `scroll-in-place' to `nil':
;;;;
...
;;;;   However, `scroll-window-in-place' is *very* confused by VM 5's message
;;;;   scrolling commands, especially because VM 5 rebuilds the Emacs window
;;;;   configuration "from scratch" so often.  I have written an experimental
;;;;   set of patches for VM 5.70 that allows VM 5 to use the "scroll-in-place"
;;;;   features; send me mail if you would like to receive a copy of these
;;;;   patches.  I hope that someday my patches will be incorporated into VM.
;;;;
;;;;   `scroll-window-in-place' can be confused by GNUS' article scrolling
;;;;   commands because they move point to the last line of the article window
;;;;   and then scroll the text.  (This is the case for at least GNUS versions
;;;;   3.13 through 4.1, inclusive.)  In addition, "in place" scrolling can
;;;;   interact poorly with the "gnus-hide" package.  "gnus-hide" can hide
...
;;;;   the "in place" scrolling commands are almost never confused, so the
;;;;   extra sanity checking isn't worth the effort.  If your mileage varies,
;;;;   let me know.
;;;;
...
vertical scrolling commands `scroll-down', `scroll-up', and `scroll-other-
window' will attempt to keep point at its current position in the window
\(window line and column).  In other words, point stays \"in place\" within the
window.
...
When this variable is non-`nil', the \"in place\" scrolling commands will
always allow blank lines to be shown past the end of the buffer.")
...
buffer-local value in the window to be scrolled.  That value is honored."
  ...
  (let* ((selected-window (selected-window))
	...
	(scroll-window-in-place other-window lines 1)
      ...
      ;; For Lucid GNU Emacs, preserve the region's state.
...
point could otherwise move the full number of lines."
  (let* (;; Make sure that the user doesn't quit in the middle and leave us
    ...
    ;; For Lucid GNU Emacs, preserve the region's state.  Note that Lucid GNU
    ;; Emacs will forcibly deactivate the region if we signal an error later
    ;; on.  Is this bad?
    (if (boundp 'zmacs-region-stays)
	(setq zmacs-region-stays t))
...
When the variable `scroll-in-place' is `nil', this function invokes the
original version of the standard GNU Emacs command `scroll-down' or
`scroll-up', as determined by DIRECTION, to scroll the window.  If DIRECTION is
-1 or `down', the original `scroll-down' is called; if DIRECTION is 1 or `up',
the original `scroll-up' is called.  Any other DIRECTION is an error.  LINES is
interpreted as if it were a raw prefix argument.  If LINES is `nil', the window
is scrolled by almost a complete windowful.  If LINES is the symbol `-', the
window is scrolled by almost a complete windowful in the opposite direction.
