...
is nil then text is shown, while if FLAG is non-nil the text is hidden.
Actually flag is really either `comment' or `block' depending on what
kind of block it is suppose to hide."
    (save-excursion
      (goto-char from)
      (end-of-line)
	  ...
	  (let ((overlay (make-overlay (point) to)))
	    ...
	    (if (eq flag 'comment)
		(overlay-put overlay 'hs-comment t))
...
(defun hs-save-state ()
  ...
  (interactive)
  (when (and hs-minor-mode hs-buffer-state-saving)
    (let ((pmax (point-max))
	  plist p opos o)
      (save-excursion
	...
	(while (/= (setq opos (next-overlay-change (point))) pmax)
	  (if (setq o (let ((look (overlays-at opos)))
	      ...
	      (let* ((o (if (listp o) (car o) o))		; kludge!
		     (beg (overlay-start o))
		     (end (overlay-end   o)))
		(if (overlay-get o 'hs-comment)
		    (setq plist (cons 'c plist)))
		(setq plist (cons (- beg p) plist))
		(setq plist (cons (- end beg) plist))
	...
	(hs-state-set buffer-file-name (nreverse plist))
...
(defun hs-restore-state ()
  ...
  (interactive)
  (when (and hs-minor-mode hs-buffer-state-saving)
    (let ((plist (cdr (hs-state-get buffer-file-name)))
	  (p (point-min))
	  flag)
      (when plist
