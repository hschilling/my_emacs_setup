...
; Modifié :  3 août 94 --------7- Martin Boyer <mboyer@ireq-robot.hydro.qc.ca>
...
will be asked to replace (y), don't replace (n), quit (q), or replace all.  When all changes are done in a file, you will be asked to
...
single line to be replaced by multiple lines."
  ...
  (let ((comment nil)
	...
	(set-buffer buf)
	(save-buffer nil))
      ;; Must always be done to keep VC in sync
      (if file-under-vc
	(vc-checkin file-under-vc nil comment))
      (if (string-match "^kill" action)
	  (kill-buffer buf))
	...
	(unless (eq next-buf buf)
	  ...
	  (if (not (vc-name file-under-vc))
	    ...
	    (let ((owner (vc-locking-user file-under-vc)))
		  ...
		  ;; I can't see how I can wait for the lock to be stealed
	...
	(while (not done)
	  (if file-editable
	    ...
	    (message
	     "According to VC, you can't edit this file.  Skipping this line.  "
	     "Hit RET")
	    (setq key (vector (read-event)))
	    (setq action 'skip))
