;;; fortran.el --- Fortran mode for GNU Emacs.  V. 1.28.6, September 24, 1992)

;; Copyright (C) 1986-1992 Free Software Foundation, Inc.
;; Written by Michael Prange.
...
;;;  Richard Stallman, Stephen Gildea, the ideas in two previous unpublished
;;;  versions of fortran .el (version 1.21.1 (from gildea@expo.lcs.mit.edu) and
;;;  1.27 (prange@erl.mit.edu).), and additions or suggestions from Mark Neale
;;;  (mjn@jet.uk), Ralf Fassel (ralf@up3aud1.gwdg.de), and Ulrich Mueller
;;;  (ulm@vsnhd2.cern.ch), and a suggestion by Dr. Anil Gokhale
;;;  (avg@dynsim1.litwin.com).  As of June 29, 1992, this list is not
;;;  complete.
...
;;; 25. One can bind tab to fortran-beautify-line to try to minimize the number
;;;     of needed continuation lines as well as the normal indenting.
;;; 

;;; Bugs to bug-fortran-mode@erl.mit.edu

(defconst fortran-mode-version "version 1.28.6")
...
(defun find-comment-start-skip ()
  "Search for comment-start-skip from point to end-of-line,
t if found, nil else.  If t, move point past comment-start-skip."
	...
	(progn
	  ...
	  (if (is-in-fortran-string-p (match-beginning 0))
		...
		(save-excursion
		  ...
		  (find-comment-start-skip))	; recursive call for rest of line
		)
...
delimited by apostrophes ('), else nil.
Quote-Escape-sequences (\\'), strings delimited by \" and
detection of syntax-errors (unbalanced quotes) are NOT supported."
...
;		     (message "in fortran string") (sit-for 1)
...
(defun fortran-beautify-line ()
  "Reformat a Fortran line, extending each line to as close to fill-column
characters as possible"
  (interactive)
  (save-excursion
    (fortran-next-statement)		;Move to beginning of statement
    (fortran-previous-statement)		;Not sure about these 2...
    (let ((cfi (calculate-fortran-indent)))
      (save-excursion
	(beginning-of-line)
	(if (or (not (= cfi (fortran-current-line-indentation)))
    ...
    (while (progn 
      ...
      (delete-horizontal-space))
    (fortran-previous-statement)
    (if (and (not (null auto-fill-hook))
	     (> (save-excursion (end-of-line) (current-column)) fill-column))
	(fortran-do-auto-fill))
    (move-to-column (calculate-fortran-indent))))

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Some experimental things, no promise they will be in future versions.
;;; The upcase/downcase and beautifier code is originally from Ralph Finch
;;; (rfinch@water.ca.gov)  The fortran-header from Larry Dodd
;;; (dodd@mycenae.cchem.berkeley.edu) has been removed.
;;;
