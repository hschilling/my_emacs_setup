Article 4128 of gnu.emacs.sources:
Path: lerc.nasa.gov!lerc.nasa.gov!purdue!news.bu.edu!bloom-beacon.mit.edu!uhog.mit.edu!news.mathworks.com!news.alpha.net!uwm.edu!vixen.cso.uiuc.edu!howland.reston.ans.net!math.ohio-state.edu!cis.ohio-state.edu!gnu.ai.mit.edu!friedman
From: friedman@gnu.ai.mit.edu (Noah Friedman)
Newsgroups: gnu.emacs.sources
Subject: comment.el --- comment out regions of buffers
Date: 12 Feb 1995 21:59:53 -0500
Organization: Source only  Discussion and requests in gnu.emacs.help.
Lines: 301
Sender: daemon@cis.ohio-state.edu
Distribution: gnu
Message-ID: <9502130256.AA10870@tepui.cli.com>
Reply-To: friedman@gnu.ai.mit.edu

Here's another set of functions of dubious value (in fact, I'm not even
certain what the return values are), but it does do more precisely what I
want than the `comment-region' function in emacs 19.
...
;; $Date: 1995/02/13 02:51:16 $|$Revision: 1.1 $|~/misc/comment.el.gz|

;; $Id: comment.el,v 1.1 1995/02/13 02:51:16 friedman Exp $
...
(defun comment-do-comment (&optional region-begin region-end count)
  (interactive "r\nP")
  (or region-begin (setq region-begin (point)))
  (or region-end (setq region-end (mark)))
  (let* ((data (match-data))
...
(defun comment-do-uncomment (&optional region-begin region-end count)
  (interactive "r\nP")
  (or region-begin (setq region-begin (point)))
  (or region-end (setq region-end (mark)))
  (let* ((data (match-data))
...
;; Try to find useful comment chars from emacs mode defaults, 
