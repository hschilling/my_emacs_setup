Article 392 of gnu.emacs.sources:
Path: eagle!usenet.ins.cwru.edu!gatech!utkcs2!ornl.gov!de5
From: de5@ornl.gov (Dave Sill)
Newsgroups: gnu.emacs.sources
Subject: lispdir.el: front-end for elisp archives and code directory
Keywords: hot hot hot
Message-ID: <1991Jul19.192824.25122@cs.utk.edu>
Date: 19 Jul 91 19:28:24 GMT
Sender: usenet@cs.utk.edu (USENET News Poster)
Reply-To: Dave Sill <de5@ornl.gov>
Organization: Oak Ridge National Laboratory
Lines: 353

Here's the latest version of lispdir.el, the front-end for the GNU
Emacs Lisp Code Directory (GELCD), and, new with this version, the
elisp archives on archive.cis.ohio-state.edu.

In a nutshell, lispdir now lets you retrieve archived files (using
ange-ftp), access the LCD remotely (using ange-ftp), and verify that
files are actually where the LCD says they ought to be.

More complete info is being posted to comp.emacs and gnu.emacs.help.
...
;; Last Modified By: Dave Sill
;; Last Modified On: Fri Jul 19 15:10:57 1991
;; Update Count    : 23
...
(defvar lisp-code-directory
 "~/gelcd/datafile"
;  "/ftp@archive.cis.ohio-state.edu:pub/gnu/emacs/elisp-archive/LCD-datafile.Z"
...
Calls value of lisp-dir-hook with no args if that value is non-nil."
   ...
   (let ((lisp-code-directory-tmp-buffer 
          (find-file-noselect lisp-code-directory)))
...
Calls value of lisp-dir-apropos-hook with no args if that value is non-nil."
  ...
  (save-excursion
    (set-buffer (get-buffer-create "*GNU Emacs Lisp Code Directory Apropos*"))
    (fundamental-mode)
    (setq buffer-read-only nil)
    (erase-buffer)
    (buffer-flush-undo (current-buffer))
